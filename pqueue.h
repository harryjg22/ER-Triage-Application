...
private:
    ...
    ******************************************/
   void deleteTree(Node* pat);
   Node* deepCopyHeap(Node* node);
   Node* merge(Node* aHeap, Node* bHeap);
   void displayHeap(Node* pat) const;
   void storePatients(Node* pat, PQueue& temp) const;
